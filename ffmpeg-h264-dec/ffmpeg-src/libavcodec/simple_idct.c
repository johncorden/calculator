...
#define R1IDCT R_FIX(0.6532814824)
#define R2IDCT R_FIX(0.2705980501)
#define R3IDCT R_FIX(0.5)
#define R_SHIFT 11
static inline void idct4row(int16_t *row)
{
    ...
    a2 = row[2];
    a3 = row[3];
    c0 = (a0 + a2)*R3IDCT + (1 << (R_SHIFT - 1));
    c2 = (a0 - a2)*R3IDCT + (1 << (R_SHIFT - 1));
    c1 = a1 * R1IDCT + a3 * R2IDCT;
    c3 = a1 * R2IDCT - a3 * R1IDCT;
    row[0]= (c0 + c1) >> R_SHIFT;
    row[1]= (c2 + c3) >> R_SHIFT;
    row[2]= (c2 - c3) >> R_SHIFT;
    row[3]= (c0 - c1) >> R_SHIFT;
